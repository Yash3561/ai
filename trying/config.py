...
def get_user_input():
    ...
    global GRID_SIZE 
    try:
        GRID_SIZE = int(input(f"Enter the size of Grid (for N x N enter only N): "))
        num_wumpus = int(input(f"Enter the number of Wumpus (max {GRID_SIZE * GRID_SIZE - 1}): "))
        num_pits = int(input(f"Enter the number of Pits (max {GRID_SIZE * GRID_SIZE - 1}): "))
        num_glitter = 1

        # Ensure user doesn't input too many objects
        if num_wumpus + num_pits + num_glitter > GRID_SIZE * GRID_SIZE - 1:
            print("Too many objects for the grid. Reducing the number of objects.")
